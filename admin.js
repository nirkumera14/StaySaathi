...
async function updateStatus(id, status) {
  const response = await fetch(
    apiUrl(`/api/admin/inquiries/${id}/status?status=${status}`),
    {
      method: "PATCH",
      headers: withHeaders(),
    },
...
async function loadDashboard() {
  try {
    ...
    const [metricsRes, inquiriesRes] = await Promise.all([
      fetch(apiUrl("/api/admin/metrics"), { headers: withHeaders() }),
      fetch(apiUrl("/api/admin/inquiries"), { headers: withHeaders() }),
    ]);
